...
var nicLinkButton=nicEditorAdvancedButton.extend({addPane:function(){this.ln=this.ne.selectedInstance.selElm().parentTag("A");this.addForm({"":{type:"title",txt:"Add/Edit Link"},href:{type:"text",txt:"URL",value:"",style:{width:"195px"}},title:{type:"text",txt:"Title",style:{width:"195px"}},target:{type:"select",txt:"Open In",options:{"":"Current Window",_blank:"New Window"},style:{width:"100px"}}},this.ln)},submit:function(C){var A=this.inputs.href.value;if(A=="http://"||A==""){alert("You must enter a URL to Create a Link");return false}this.removePane();if(!this.ln){var B="javascript:nicTemp();";this.ne.nicCommand("createlink",B);this.ln=this.findElm("A","href",B)}if(this.ln){this.ln.setAttributes({href:this.inputs.href.value,title:this.inputs.title.value,target:this.inputs.target.options[this.inputs.target.selectedIndex].value})}}});nicEditors.registerPlugin(nicPlugin,nicLinkOptions);



var nicJoomlaOptions = {

	buttons : {

		'joomla' : {name : 'Add Joomla Menu Link', type : 'nicJoomlaButton', tags : ['A']},
...
var nicJoomlaButton=nicEditorAdvancedButton.extend({addPane:function(){this.ln=this.ne.selectedInstance.selElm().parentTag("A");this.addForm({"":{type:"title",txt:"Add/Edit Joomla Link"},href:{type:"text",txt:"URL",value:"",style:{width:"195px"}},title:{type:"text",txt:"Title",style:{width:"195px"}},target:{type:"select",txt:"Open In",options:{"":"Current Window",_blank:"New Window"},style:{width:"100px"}}},this.ln)},submit:function(C){var A=this.inputs.href.value;if(A=="index.php?Itemid="||A==""){alert("You must enter a valid Joomla Menu Link");return false}this.removePane();if(!this.ln){var B="javascript:nicTemp();";this.ne.nicCommand("createlink",B);this.ln=this.findElm("A","href",B)}if(this.ln){this.ln.setAttributes({href:this.inputs.href.value,title:this.inputs.title.value,target:this.inputs.target.options[this.inputs.target.selectedIndex].value})}}});nicEditors.registerPlugin(nicPlugin,nicJoomlaOptions);
...
var nicImageButton=nicEditorAdvancedButton.extend({addPane:function(){this.im=this.ne.selectedInstance.selElm().parentTag("IMG");this.addForm({"":{type:"title",txt:"Add/Edit Image"},src:{type:"text",txt:"URL",value:"",style:{width:"150px"}},alt:{type:"text",txt:"Alt Text",style:{width:"100px"}},align:{type:"select",txt:"Align",options:{none:"Default",left:"Left",right:"Right"}}},this.im)},submit:function(B){var C=this.inputs.src.value;if(C==""||C=="http://"){alert("You must enter a Image URL to insert");return false}this.removePane();if(!this.im){this.ne.nicCommand("insertHTML",'<img align="'+this.inputs.align.value+'" alt="'+this.inputs.alt.value+'" src="'+this.inputs.src.value+'"> ');}}});nicEditors.registerPlugin(nicPlugin,nicImageOptions);
...
bkLib.onDomLoaded(function() {
	...
	var buttons = new Array('fontFamily','fontFormat','fontSize','bold','italic','underline','strikethrough','left','center','right','justify','subscript','superscript','ol','ul','indent','outdent','hr','forecolor','bgcolor','link','unlink','image','removeformat','xhtml');

	

	new nicEditor({maxHeight : 260, iconsPath : '../wp-content/plugins/editor4roks-wordpress/assets/img/nicEditorIcons.gif', buttonList : buttons }).panelInstance('e4r-textarea');

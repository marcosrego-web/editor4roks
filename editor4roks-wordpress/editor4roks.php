...
function e4r_select_field_3_render(  ) { 
	...
	<label><input type="checkbox" name="e4r_settings[e4r_select_field_3][]" value="4" <?php checked( ( is_array( $linkstypes ) AND in_array( "4", $linkstypes ) ) ? "4" : '', "4" ); ?>>Documents<br></label>
	<label><input type="checkbox" name="e4r_settings[e4r_select_field_3][]" value="5" <?php checked( ( is_array( $linkstypes ) AND in_array( "5", $linkstypes ) ) ? "5" : '', "5" ); ?>>Images<br></label>
...
function e4r_options_page(  ) { 
	...
	<form action='options.php' method='post'>
		...
		<a href="https://rockettheme.com/wordpress/plugins/roksprocket" target="_blank" class="button">Roksprocket on RocketTheme website</a>
			...
			if(is_array( $linkstypes ) && in_array(4, $linkstypes)) {
				foreach($getFiles as $document){
					if(substr($document, -4) == '.pdf' || substr($document, -4) == '.PDF' || substr($document, -4) == '.doc' || substr($document, -4) == '.DOC' || substr($document, -5) == '.docx' || substr($document, -5) == '.DOCX' || substr($document, -4) == '.xls' || substr($document, -4) == '.XLS' || substr($document, -5) == '.xlsx' || substr($document, -5) == '.XLSX' || substr($document, -4) == '.odt' || substr($document, -4) == '.ODT'|| substr($image, -4) == '.PPT'|| substr($image, -4) == '.ppt') {
						...
						echo '<option value="'.$rootUrl.$document.'">📋 document - '.$documentname.'</option>';
					}
				}
			}
			//'Images' links type
